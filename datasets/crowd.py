...
def crowd_collate(batch):
    """
    Collate function for Crowd dataset (train mode).
    Handles variable-length keypoints/targets.
    """
    ...
    for rgb, t, keypoints, target, st_size in batch:
        rgb_list.append(rgb)
        t_list.append(t)
        keypoints_list.append(keypoints)   # variable length
        targets_list.append(target)        # variable length
...
def crowd_collate_val(batch):
    """
    Collate function for Crowd dataset (val/test mode).
    """
    rgb_list, t_list, target_list, counts, names = [], [], [], [], []

    for rgb, t, target, count, name in batch:
        rgb_list.append(rgb)
        t_list.append(t)
        target_list.append(torch.from_numpy(target).float() if not torch.is_tensor(target) else target)
        counts.append(count)
        names.append(name)
    ...
    return rgb, t, target_list, counts, names
